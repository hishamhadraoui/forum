...
$(document).ready(function(){
	...
	/* Data readit Starts Here */
	$(".readit").click(function()
	{
		var id = $(this).attr("id");
		var read_id = id;
		var parent = $(this).parent("td").parent("tr");
		if(confirm('هل أنت متأكد من أنك تريد قراءة الإشعار'))
		{
			$.post('post.php?type=readit', {'read_id':read_id}, function(data)
			{
				//parent.fadeOut('slow');
				//confirm('تم قراءة الإشعار بنجاح');
	...
	$(".dsticky-topic").click(function()
	{
		...
		var dstic_id = id;
		var parent = $(this).parent("td").parent("tr");
		if(confirm('هل أنت متأكد أنك تريد إلغاء حذف الموضوع'))
		{
			$.post('post.php?type=dsticky', {'dstic_id':dstic_id}, function(data)
			{
				//parent.fadeOut('slow');
				confirm('تم إلغاء الحذف ينجاح');
				document.location.reload(true);
